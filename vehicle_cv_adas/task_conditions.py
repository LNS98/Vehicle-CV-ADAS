...
from .traffic_lane_detector.ufldDetector.utils import OffsetType, CurvatureType

STD_OUTPUT_HANDLE = -11
std_out_handle = False  # ctypes.windll.kernel32.GetStdHandle(STD_OUTPUT_HANDLE)


def set_color(color, handle=std_out_handle):
    bool = False  # ctypes.windll.kernel32.SetConsoleTextAttribute(handle, color)
    return bool
...
class TaskConditions(object):
    ...
    def _calc_deviation(self, offset, offset_thres):
        ...
        if abs(offset) > offset_thres:
            if offset > 0 and self.curvature_msg not in {
                CurvatureType.HARD_LEFT,
                CurvatureType.EASY_LEFT,
            }:
                msg = OffsetType.RIGHT
            elif offset < 0 and self.curvature_msg not in {
                CurvatureType.HARD_RIGHT,
                CurvatureType.EASY_RIGHT,
            }:
                msg = OffsetType.LEFT
            else:
    ...
    def CheckStatus(self):
        ...
        if (
            self.curvature_msg == CurvatureType.UNKNOWN
            and self.offset_msg == OffsetType.UNKNOWN
        ):
            self.toggle_oscillator_status = [False, False]
    ...
    def UpdateRouteStatus(self, vehicle_direction, vehicle_curvature, curvae_thres=500):
        ...
        if vehicle_curvature != None:
            if vehicle_direction != None and self.offset_msg == OffsetType.CENTER:
                self.vehicle_curvature_record.append(
                    [vehicle_direction, vehicle_curvature]
                )

                if self.vehicle_curvature_record.full():
                    avg_direction = max(
                        set(np.squeeze(self.vehicle_curvature_record)[:, 0]),
                        key=self.vehicle_curvature_record.count,
                    )
                    avg_curvature = np.median(
                        [
                            int(float(i))
                            for i in np.array(self.vehicle_curvature_record)[:, 1]
                        ]
                    )
                    self.curvature_msg = self._calc_direction(
                        avg_curvature, avg_direction, curvae_thres
                    )

                    if self.toggle_status_counter["Curvae"] >= 10:
                        if (
                            self.curvature_msg != CurvatureType.STRAIGHT
                            and abs(self.vehicle_offset_record[-1]) < 0.2
                            and not np.array(self.toggle_oscillator_status).any()
                        ):
